...
# Thiết lập logging
logging.basicConfig(level=logging.INFO, filename='howi_mto_week38_step2.log', filemode='w',
...
def calculate_WIO_WIOUB(itemset, fp_tree, TO, weight_dict):
    ...
    # Tính WIO
    WIO = sum(TO[tid] * sum(weight_dict[item] for item in itemset) / len(itemset) 
    ...
    # Tính WIOUB
    WIOUB = sum(TO[tid] * max(weight_dict[item] for item in itemset) 
...
def fp_growth(fp_tree, TO, weight_dict, MinWIO, prefix=None, HOI=None):
    ...
    items = [(item, estimate_WIOUB(item, fp_tree, TO, weight_dict)) 
             for item in fp_tree.header_table.keys() 
             if estimate_WIOUB(item, fp_tree, TO, weight_dict) >= MinWIO]
...
def tune_parameters(transactions_file, weights_file):
    ...
    min_wio_values = [0.01, 0.05, 0.1, 0.15, 0.2]
    min_ws_values = [0.01, 0.05, 0.1, 0.15, 0.2]
    ...
    results_df.to_csv('result_week38_step2.csv', index=False)
    logging.info("Results saved to result_week38_step2.csv")
    print("\nResults saved to result_week38_step2.csv")
